...
dfsim2=pd.read_csv(plotday + ' Similarity session2session3rotated.csv')
dfsim2.set_index('Unnamed: 0',inplace=True)

dfcompare=pd.DataFrame(columns=dfsim1.columns)
dfcompare.loc['Light rotated']=dfsim1.loc[0]
dfcompare.loc['Short light rotated']=dfsim2.loc[0]
for cell in dfcompare.columns:
    if dfcompare[cell].isnull().values.any():
        dfcompare.drop(columns=cell,inplace=True)
TTestStats,TTestP=stats.ttest_rel(dfcompare.loc['Light rotated'],dfcompare.loc['Short light rotated'])
#%% T test of similarity, visual cue rotation
plotday='0729'
dfsim1=pd.read_csv(plotday + ' Rotation session0session1.csv')
dfsim1.set_index('Unnamed: 0',inplace=True)
dfsim2=pd.read_csv(plotday + ' Rotation session0session2.csv')
dfsim2.set_index('Unnamed: 0',inplace=True)
...
plt.savefig(plotday + ' Visual cue rotation.png', bbox_inches='tight')
#%% Rotation analysis
plotday='0805'
with open(plotday + ' CellMaps.json', 'r') as f:
    CellMaps = json.load(f)
...
plotday='0715'
dfrotmax=pd.read_csv('0715 Rotation Count session2session4.csv')
dfrotmax.set_index('Unnamed: 0',inplace=True)   
plt.figure()
sns.barplot(data=dfrotmax.T, x=['0','90','180','270'],y='Counts')
plt.bar(x=['0','90','180','270'],height=dfrotmax.T['Counts'],edgecolor='k',linewidth=2)
...
plt.savefig(plotday + ' Rotation Count session3session4.png', bbox_inches='tight')
#%% Distinction index
'''
Manual input
'''
